...
const PrintTimetable = React.forwardRef(({ section }, ref) => {
    ...
    if (loading) {
        return (
            <div ref={ref} style={{ padding: 40, textAlign: 'center' }}>
                <Spin size="large" tip="Loading timetable sheet..." />
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div ref={ref} style={{ padding: 30, textAlign: 'center', color: '#ef4444' }}>
                <Title level={4} style={{ color: '#ef4444' }}>Error Loading Timetable</Title>
                <Text>{error}</Text>
            </div>
        );
    }
    ...
    return (
        <div ref={ref} className="printable-timetable-container" style={{
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <div><strong>SECTION:</strong> <span style={{ background: '#0b1b3d', color: '#d4af37', padding: '2px 8px', borderRadius: 4, fontWeight: 700 }}>Section {section?.name || 'All'}</span></div>
